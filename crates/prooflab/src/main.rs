    ...
    match &cli.command {
        ...
        Commands::ProveSp1(args) => {
            ...
            if let Some(size) = args.benchmark_size {
                ...
                // Sweep mode
                info!(
                    "Running benchmark sweep from {} to {} with step {}",
...
async fn run_sp1_proof(args: &ProofArgs) -> io::Result<()> {
    let telemetry = TelemetryCollector::new(
        "SP1",
        args.precompiles,
        args.gpu,
    ...
    if utils::validate_directory_structure(&args.guest_path) {
        ...
        if let Some(code) = result.code() {
            match code {
                101 => {
                    error!("Proof verification failed - the generated proof could not be verified")
                }
...
async fn run_risc0_proof(args: &ProofArgs) -> io::Result<()> {
    let telemetry = TelemetryCollector::new(
        "RISC0",
        args.precompiles,
        args.gpu,
