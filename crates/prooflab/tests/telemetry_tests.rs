...
fn test_telemetry_collector_creation() {
    ...
    assert!(collector.finalize().is_none(), "Telemetry data should be None when enable_telemetry is false");
}
...
fn test_telemetry_collector_with_telemetry_enabled() {
    ...
    collector.record_proof_generation(Duration::from_secs(3));
    ...
    assert!(telemetry_data.is_some(), "Telemetry data should be Some when enable_telemetry is true");
    
    // Check telemetry data
    let data = telemetry_data.unwrap();
    
    // Check timing fields
    assert_eq!(data.timing.workspace_setup_duration, Some(Duration::from_secs(1)));
    assert_eq!(data.timing.compilation_duration, Some(Duration::from_secs(2)));
    assert_eq!(data.timing.proof_generation_duration, Some(Duration::from_secs(3)));
    ...
    assert_eq!(data.timing.core_prove_duration, Some(Duration::from_secs(4)));
    assert_eq!(data.timing.core_verify_duration, Some(Duration::from_secs(1)));
    assert_eq!(data.timing.compress_prove_duration, Some(Duration::from_secs(2)));
    assert_eq!(data.timing.compress_verify_duration, Some(Duration::from_secs(1)));
